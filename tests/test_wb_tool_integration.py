...
def test_resolve_product_images_uses_wb_content_api(monkeypatch) -> None:
    async def fake_context(self, sku_id: int, account_id: int | None = None) -> dict:
        return {"sku_id": sku_id, "nm_id": 4242, "title": "Товар", "images": []}
    ...
    async def fake_content(self, nm_id: int) -> list[str]:
        assert nm_id == 4242
        return ["https://content.example/1.jpg", "https://content.example/2.jpg"]

    monkeypatch.setattr(settings, "wb_content_api_token", "content-token")
    monkeypatch.setattr(WBToolClient, "product_context", fake_context)
    monkeypatch.setattr(WBToolClient, "product_media_urls", fake_media)
    monkeypatch.setattr(
    ...
    assert [img.url for img in product.images] == [
        "https://content.example/1.jpg",
        "https://content.example/2.jpg",
    ]
...
def test_resolve_product_images_without_content_token_uses_cdn(monkeypatch) -> None:
    ...
    async def fail_content(self, nm_id: int) -> list[str]:
        raise AssertionError("WB Content API must not be called without a token")
    ...
    assert all(img.alt.startswith("WB CDN") for img in product.images)
    assert "CDN Wildberries" in notes[0]
